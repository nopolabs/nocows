...
BloomFilter.prototype.add = function(v) {
    ...
    for (var i = 0; i < k; ++i) {
        console.log(l[i], Math.floor(l[i] / 32), l[i] % 32);
...
BloomFilter.prototype.toBase64 = function() {
    const view = new DataView(this.buckets.buffer)
    var binary = '';
...
BloomFilter.prototype.fromBase64 = function(b64) {
    const view = new DataView(this.buckets.buffer)
    var binary_string = atob(b64);
    var len = binary_string.length;
    for (var i = 0; i < len; i++) {
        view.setUint8(i, binary_string.charCodeAt(i));
    }
    return this
}
